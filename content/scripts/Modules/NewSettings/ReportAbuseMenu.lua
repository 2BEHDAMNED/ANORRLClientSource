...
local function Initialize()
	local settingsPageFactory = require(ANORRLGui.Modules.NewSettings.SettingsPageFactory)
	...
	function this:SetHub(newHubRef)
		...
		this.Page.ZIndex = newHubRef.ZIndex + 2
		...
		local function makeSubmitButtonActive()
			submitButton.ZIndex = this.Page.ZIndex + 2
			submitButton.Selectable = true
			submitText.ZIndex = this.Page.ZIndex + 2
		end

		local function makeSubmitButtonInactive()
			submitButton.ZIndex = this.Page.ZIndex + 1
			submitButton.Selectable = false
			submitText.ZIndex = this.Page.ZIndex + 1
		end
		...
		local function updateAbuseDropDown()
			...
			if this.GameOrPlayerMode.CurrentIndex == 1 then
				this.TypeOfAbuseMode:UpdateDropDownList(ABUSE_TYPES_GAME)
				this.WhichPlayerMode:SetZIndex(this.Page.ZIndex - 1)
				this.WhichPlayerMode:SetInteractable(false)
				this.WhichPlayerLabel.ZIndex = this.Page.ZIndex - 1
